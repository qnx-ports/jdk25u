...
address os::Posix::ucontext_get_pc(const ucontext_t * uc) {
  return (address)uc->uc_mcontext.cpu.REG_PC;
}

void os::Posix::ucontext_set_pc(ucontext_t * uc, address pc) {
  uc->uc_mcontext.cpu.REG_PC = (intptr_t)pc;
}

intptr_t* os::Qnx::ucontext_get_sp(const ucontext_t * uc) {
  return (intptr_t*)uc->uc_mcontext.cpu.REG_SP;
}

intptr_t* os::Qnx::ucontext_get_fp(const ucontext_t * uc) {
  return (intptr_t*)uc->uc_mcontext.cpu.REG_FP;
}
...
address os::fetch_frame_from_context(const void* ucVoid,
  ...
  if (uc != nullptr) {
    ...
    if (ret_sp) *ret_sp = os::Qnx::ucontext_get_sp(uc);
    if (ret_fp) *ret_fp = os::Qnx::ucontext_get_fp(uc);
  } else {
    epc = nullptr;
...
frame os::fetch_compiled_frame_from_context(const void* ucVoid) {
  const ucontext_t* uc = (const ucontext_t*)ucVoid;
  intptr_t* fp = os::Qnx::ucontext_get_fp(uc);
  intptr_t* sp = os::Qnx::ucontext_get_sp(uc);
...
intptr_t* os::fetch_bcp_from_context(const void* ucVoid) {
  ...
  return reinterpret_cast<intptr_t*>(uc->uc_mcontext.cpu.REG_BCP);
}
...
// qnx 8 only support x86_64 no need to do anything here
void os::Qnx::init_thread_fpu_state(void) {}
...
juint os::cpu_microcode_revision() {
  ...
  // Qnx currently does not have a way to get the microcode
  // that being said it appears that this info is for debug purpose
  // and freebsd also retuns 0
...
void os::print_context(outputStream *st, const void *context) {
  ...
  st->print(  "RAX=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rax);
  st->print(", RBX=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rbx);
  st->print(", RCX=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rcx);
  st->print(", RDX=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rdx);
  st->cr();
  st->print(  "RSP=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rsp);
  st->print(", RBP=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rbp);
  st->print(", RSI=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rsi);
  st->print(", RDI=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rdi);
  st->cr();
  st->print(  "R8 =" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r8);
  st->print(", R9 =" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r9);
  st->print(", R10=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r10);
  st->print(", R11=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r11);
  st->cr();
  st->print(  "R12=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r13);
  st->print(", R13=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r14);
  st->print(", R14=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r15);
  st->print(", R15=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.r15);
  st->cr();
  st->print(  "RIP=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rip);
  st->print(", EFLAGS=" INTPTR_FORMAT, (intptr_t)uc->uc_mcontext.cpu.rflags);
...
void os::print_register_info(outputStream *st, const void *context, int& continuation) {
  ...
  while (n < register_count) {
    ...
    continuation = n + 1;
# define CASE_PRINT_REG(n, str, id) case n: st->print(str); print_location(st, uc->uc_mcontext.cpu.id);
  switch (n) {
