...
////////////////////////////////////////////////////////////////////////////////
// global variables
julong os::Qnx::_physical_memory = 0;

int (*os::Qnx::_getcpuclockid)(pthread_t, clockid_t *) = &pthread_getcpuclockid;
pthread_t os::Qnx::_main_thread;

////////////////////////////////////////////////////////////////////////////////
// utility functions

julong os::available_memory() {
  return Qnx::available_memory();
}
...
julong os::Qnx::available_memory() {
  ...
  available = msg.o.info.posix_tmi_length;
...
void os::Qnx::print_uptime_info(outputStream* st) {
	//TODO: fixme
...
julong os::physical_memory() {
  return Qnx::physical_memory();
}

size_t os::rss() {

  mem_as_info_t   msg;
  msg.i = (struct _mem_as_info){ .type = _MEM_AS_INFO };
...
void os::Qnx::initialize_system_info() {
  
  set_processor_count((int)_syspage_ptr->num_cpu);
  ...
  _physical_memory = msg.o.info.__posix_tmi_total;
}
...
#define EXTENSIONS_DIR  "/lib/ext"
  ...
  // on legacy Qnx implementations (which are no longer supported).
...
static void *thread_native_entry(Thread *thread) {
  ...
  osthread->set_thread_id(os::Qnx::gettid());

  // initialize signal mask for this thread
  PosixSignals::hotspot_sigmask(thread);

  // initialize floating point control register
  os::Qnx::init_thread_fpu_state();
...
bool os::create_main_thread(JavaThread* thread) {
  assert(os::Qnx::_main_thread == pthread_self(), "should be called inside main thread");
...
// QNX resources related to the OSThread
...
void *os::Qnx::dlopen_helper(const char *filename, char *ebuf, int ebuflen) {
#ifndef IA32
...
void * os::dll_load(const char *filename, char *ebuf, int ebuflen) {
  ...
  void* result;
  result = os::Qnx::dlopen_helper(filename, ebuf, ebuflen);
  if (result != nullptr) {
    return result;
  }
...
static int dl_iterate_callback(const struct dl_phdr_info *info, size_t size, void *data) {
...
void os::pd_disclaim_memory(char *addr, size_t bytes) {
  ::posix_madvise(addr, bytes, POSIX_MADV_DONTNEED);
}
...
bool os::pd_uncommit_memory(char* addr, size_t size, bool exec) {
  if (exec) {
    if (::posix_madvise(addr, size, POSIX_MADV_DONTNEED) != 0) {
      ErrnoPreserver ep;
...
// may not start from the requested address. Unlike Qnx mmap(), this
...
#if defined(__QNX__) && defined(MAP_EXCL)
...
static bool qnx_mprotect(char* addr, size_t size, int prot) {
  // Qnx wants the mprotect address argument to be page aligned.
...
bool os::protect_memory(char* addr, size_t bytes, ProtType prot,
  ...
  return  qnx_mprotect(addr, bytes, p);
}

bool os::guard_memory(char* addr, size_t size) {
  return  qnx_mprotect(addr, size, PROT_NONE);
}

bool os::unguard_memory(char* addr, size_t size) {
  return  qnx_mprotect(addr, size, PROT_READ|PROT_WRITE);
}

bool os::Qnx::hugetlbfs_sanity_check(bool warn, size_t page_size) {
  return false;
}
...
char* os::pd_attempt_reserve_memory_at(char* requested_addr, size_t bytes, bool exec) {
  ...
  // Qnx mmap allows caller to pass an address as hint; give it a try first,
...
// Note: Normal Qnx applications are run with SCHED_OTHER policy. SCHED_OTHER
// only supports dynamic priority, static priority must be zero. For real-time
// applications, Qnx supports SCHED_RR which allows static priority (1-99).
...
void os::init(void) {
  ...
  if (os::vm_page_size() == 0) {
    fatal("os_qnx.cpp: os::init: getpagesize() failed (%s)", os::strerror(errno));
  }
  _page_sizes.add(os::vm_page_size());

  Qnx::initialize_system_info();

  // _main_thread points to the thread that created/loaded the JVM.
  Qnx::_main_thread = pthread_self();
...
jint os::init_2(void) {
  ...
  if (MaxFDLimit) {
    ...
    if (status != 0) {
      ...
      // to hang.
      nbr_files.rlim_cur = __min(0x100000, nbr_files.rlim_max);
      ...
      if (status != 0) {
        ...
        nbr_files.rlim_cur = __max(_POSIX_OPEN_MAX, rlim_original);
        status = setrlimit(RLIMIT_NOFILE, &nbr_files);
      }
      if (status != 0) {
...
// This does not do anything on Qnx. This is basically a hook for being
...
jlong os::thread_cpu_time(Thread *thread, bool user_sys_cpu_time) {
  if (user_sys_cpu_time && Qnx::_getcpuclockid != nullptr) {
    struct timespec tp;
    clockid_t clockid;
    int ret;
     ...
     */
    ret = Qnx::_getcpuclockid(thread->osthread()->pthread_id(), &clockid);
    if (ret != 0)
      return -1;
...
bool os::is_thread_cpu_time_supported() {
  return (Qnx::_getcpuclockid != nullptr);
}

// System loadavg support.  Returns -1 if load average cannot be obtained.
// Qnx doesn't yet have a way to get the system avg so we just always return -1
...
void os::current_stack_base_and_size(address* base, size_t* size) {
  ...
  address bottom = static_cast<address>(tls->__stackaddr);
  *size = tls->__stacksize;
  *base = bottom + tls->__stacksize;
